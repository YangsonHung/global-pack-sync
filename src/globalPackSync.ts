...
import { installPackagesConcurrently } from './utils/installer';
import type {
  ColorPalette,
  CommonOptions,
  Config,
...
class GlobalPackSync {
  ...
  constructor(options: GlobalPackSyncOptions = {}) {
    ...
    this.lockFile = path.join(configDir, LOCK_FILE_NAME);
    this.colors = { ...DEFAULT_COLORS };
    this.skipPackages = new Set(DEFAULT_SKIP_PACKAGES);
  }
  ...
  private checkLock(): void {
    const existingLock = readLockFile(this.lockFile);
    if (existingLock) {
      ...
      clearLockFile(this.lockFile);
    }

    this.ensureConfigDir();
    writeLockFile(this.lockFile, {
      pid: process.pid,
      timestamp: Date.now(),
    });
  ...
  public async getGlobalPackages(packageManager: PackageManager = 'npm'): Promise<PackageMap> {
    try {
      return listGlobalPackages(packageManager, this.skipPackages);
    } catch (error) {
  ...
  public async save(profileName: string | null = null, options: CommonOptions = {}): Promise<void> {
    ...
    try {
      ...
      const config = readJsonFile<Config>(this.configFile) ?? {};
      ...
      backupFileIfExists(this.configFile);
  ...
  public async installPackagesConcurrently(
    ...
    return installPackagesConcurrently({
      packages,
      packageManager,
      concurrency,
      useLatestVersion,
      colors: this.colors,
      getLatestVersion: (name, pm) => this.getLatestVersion(name, pm),
    });
  }
